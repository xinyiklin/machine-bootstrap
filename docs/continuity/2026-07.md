# Continuity Archive — 2026-07

Rotated history for the Machine Bootstrap ledger. Append-only. Not read at task
start; read only when a task needs historical context. The live ledger is
`CONTINUITY.md` in the repository root.

Entries are preserved verbatim from the ledger's `Snapshot` section at rotation
time (2026-08-14). Provenance tags and dates are unchanged.

## 2026-07-27 — Bootstrap Foundation
...
## 2026-07-27 — Installer Hardening And Integrity
...
## 2026-07-31 — Product Delivery Workflow Foundation
